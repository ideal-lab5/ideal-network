...
use crate::{
    ...
    worker::{PersistedState, VoterOracle},
...
impl BeefyTestNet {
    ...
    pub(crate) async fn generate_blocks_and_sync(
        ...
        let built_hashes = self
            .peer(0)
            .generate_blocks(count, BlockOrigin::File, |mut builder| {
...
impl TestNetFactory for BeefyTestNet {
    ...
    fn make_block_import(
        ...
        #[cfg(not(feature = "bls-experimental"))]
        let validator_set =
            ValidatorSet::new(make_beefy_ids(keys), 0).unwrap();
    ...
    API::Api: BeefyApi<Block, AuthorityId> + MmrApi<Block, MmrRootHash, NumberFor<Block>>,
{
    ...
    let mut notification_services = peers
        ...
        .map(|(peer_id, _, _)| {
            ...
            (
                *peer_id,
                peer.take_notification_service(&beefy_gossip_proto_name())
                    .unwrap(),
            )
        })
...
pub(crate) fn get_beefy_streams(
    ...
    Vec<NotificationReceiver<BeefyVersionedFinalityProof<Block>>>,
) {
...
async fn wait_for_best_beefy_blocks(
    ...
    streams.into_iter().enumerate().for_each(|(i, stream)| {
        ...
        wait_for.push(Box::pin(stream.take(len).for_each(
            move |best_beefy_hash| {
            ...
            move |versioned_finality_proof| {
                ...
                async move {
                    let signed_commitment = match versioned_finality_proof {
                        sp_consensus_beefy_etf::VersionedFinalityProof::V1(sc) => sc,
                    };
...
async fn beefy_finalizing_blocks() {
    ...
    #[cfg(not(feature = "bls-experimental"))]
    let validator_set =
        ValidatorSet::new(make_beefy_ids(&peers), 0).unwrap();
    let session_len = 10;
    ...
    let beefy_peers = peers
        .iter()
        .enumerate()
        .map(|(id, key)| (id, key, api.clone()))
        .collect();
    ...
    let hashes = net
        .generate_blocks_and_sync(42, session_len, &validator_set, true)
        .await;
...
async fn lagging_validators() {
    ...
    let hashes = net
        .generate_blocks_and_sync(62, session_len, &validator_set, true)
        .await;
...
async fn correct_beefy_payload() {
    ...
    // push 12 blocks
    let hashes = net
        .generate_blocks_and_sync(12, session_len, &validator_set, false)
        .await;
...
async fn beefy_importing_justifications() {
    ...
    let justif_stream = beefy_voter_links
        .lock()
        .take()
        .unwrap()
    ...
    assert_eq!(
        block_import
            .import_block(params(block.clone(), None))
            .await
            .unwrap(),
    ...
    assert_eq!(
        block_import
            .import_block(params(block, None))
            .await
            .unwrap(),
    ...
    assert_eq!(
        block_import
            .import_block(params(block, justif))
            .await
            .unwrap(),
        ImportResult::Imported(ImportedAux {
...
async fn on_demand_beefy_justification_sync() {
    ...
    // push 30 blocks
    let mut hashes = net
        .generate_blocks_and_sync(30, session_len, &validator_set, false)
        .await;
    ...
    // Dave pushes and syncs 4 more blocks just to make sure he gets included in gossip.
    {
        ...
        let built_hashes =
            net_guard
                .peer(dave_index)
                .generate_blocks(4, BlockOrigin::File, |builder| {
    ...
    let (dave_best_blocks, _) = get_beefy_streams(
        &mut net.lock(),
...
async fn should_initialize_voter_at_genesis() {
    ...
    let persisted_state = voter_init_setup(&mut net, &mut finality, &api)
        .await
        .unwrap();
...
async fn should_initialize_voter_at_custom_genesis() {
    ...
    let hashes = net
        .generate_blocks_and_sync(15, 15, &validator_set, false)
        .await;
    let mut finality = net
        .peer(0)
        .client()
    ...
    net.peer(0)
        .client()
        .as_client()
        .finalize_block(hashes[3], None)
        .unwrap();
    net.peer(0)
        .client()
        .as_client()
        .finalize_block(hashes[5], None)
        .unwrap();
    net.peer(0)
        .client()
        .as_client()
        .finalize_block(hashes[8], None)
        .unwrap();
    ...
    assert_eq!(
        persisted_state.voting_oracle().voting_target(),
    ...
    net.peer(0)
        .client()
        .as_client()
        .finalize_block(hashes[10], None)
        .unwrap();
    // load persistent state - state preset in DB, but with different pallet genesis
    let new_persisted_state = voter_init_setup(&mut net, &mut finality, &api)
        .await
        .unwrap();
    ...
    assert_eq!(
        new_persisted_state.voting_oracle().voting_target(),
        Some(new_pallet_genesis)
    );
...
async fn should_initialize_voter_at_custom_genesis_when_state_unavailable() {
    ...
    let hashes = net
        .generate_blocks_and_sync(30, 5, &validator_set, false)
        .await;
    let mut finality = net
        .peer(0)
        .client()
...
async fn should_catch_up_when_loading_saved_voter_state() {
    ...
    let hashes = net
        .generate_blocks_and_sync(30, 10, &validator_set, false)
        .await;
    let mut finality = net
        .peer(0)
        .client()
...
async fn beefy_reports_equivocations() {
    ...
    // push 42 blocks including `AuthorityChange` digests every 10 blocks.
    let hashes = net
        .generate_blocks_and_sync(42, session_len, &validator_set, false)
        .await;
...
async fn gossipped_finality_proofs() {
    ...
    let mut charlie_gossip_engine = sc_network_gossip::GossipEngine::new(
        ...
        charlie
            .take_notification_service(&beefy_gossip_proto_name())
            .unwrap(),
    ...
    let vote_message = VoteMessage {
        commitment,
        id: BeefyKeyring::Charlie.public(),
        signature,
    };
    ...
    let mut charlie_gossip_proofs = Box::pin(
        charlie_gossip_engine
            ...
            .filter_map(|notification| async move {
                GossipMessage::<Block>::decode(&mut &notification.message[..])
                    .ok()
