...
pub fn beefy_block_import_and_links<B, BE, RuntimeApi, I>(
    ...
    BeefyBlockImport<B, BE, RuntimeApi, I>,
    BeefyVoterLinks<B>,
    BeefyRPCLinks<B>,
)
where
    ...
    S: GossipSyncing<B> + SyncOracle + 'static,
{
    ...
    let mut block_import_justif = links
        .from_block_import_justif_stream
        .subscribe(100_000)
        .fuse();
    ...
    if let Some(ref keystore) = key_store {
    ...
    BC: BlockchainBackend<B>,
{
    if *current.number() == Zero::zero() {
        let msg = format!(
            "header {} is Genesis, there is no parent for it",
            current.hash()
        );
