    ...
    R::Api: BeefyApi<B, AuthorityId>,
{
    ...
    fn finalize(&mut self, finality_proof: BeefyVersionedFinalityProof<B>) -> Result<(), Error> {
        ...
        self.comms
            ...
            .cancel_requests_older_than(block_num);
    ...
    fn try_pending_justifications(&mut self) -> Result<(), Error> {
        ...
        if !self.pending_justifications.is_empty() {
            ...
            let still_pending = self
                ...
                .split_off(&end.saturating_add(1u32.into()));
    ...
    fn do_vote(&mut self, target_number: NumberFor<B>) -> Result<(), Error> {
            ...
            })?
        {
            ...
            if let Some(finality_proof) = self.handle_vote(vote.clone()).map_err(|err| {
                ...
                self.comms
                    .gossip_engine
                    .gossip_message(votes_topic::<B>(), encoded_vote, false);
            }
    ...
    #[cfg(feature = "bls-experimental")]
    fn get_signed_payload(
        &mut self,
        target_number: NumberFor<B>,
        target_header: B::Header,
        validator_set_id: ValidatorSetId,
        authority_id: AuthorityId,
    ) -> Result<Option<(Signature, AuthorityId, Commitment<NumberFor<B>>)>, Error> {
        let target_hash = target_header.hash();
        ...
        Ok(Some((signature, etf_authority_id, commitment)))
    }
    ...
    fn get_signed_payload(
        ...
        Ok(Some((signature, authority_id, commitment)))
    }
...
pub(crate) mod tests {
    ...
    > {
        ...
        let beefy_rpc_links = BeefyRPCLinks {
    ...
    fn test_oracle_accepted_interval() {
        ...
        for i in 0..15 {
            assert_eq!(
                accepted_interval_with(&mut oracle, i),
                Ok((session_one, session_one))
            );
        }
        ...
        for i in session_three..session_three + 15 {
            assert_eq!(
                accepted_interval_with(&mut oracle, i),
                Ok((session_three, session_three))
            );
        }
        ...
        assert_eq!(
            oracle.sessions.front().unwrap().session_start(),
            session_four
        );
        assert_eq!(
    ...
    async fn should_finalize_correctly() {
        ...
        let create_finality_proof = |block_num: NumberFor<Block>| {
            ...
            VersionedFinalityProof::V1(SignedCommitment {
                commitment,
                signatures: vec![None],
            })
        };
        ...
        worker
            .persisted_state
            .voting_oracle
            .add_session(Rounds::new(2, validator_set));
