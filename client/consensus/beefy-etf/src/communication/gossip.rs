...
impl<B: Block> Filter<B> {
    pub fn new() -> Self {
        Self {
            inner: None,
            rounds_with_valid_proofs: BTreeSet::new(),
        }
    }
    ...
    B: Block,
{
    ...
    fn message_expired<'a>(&'a self) -> Box<dyn FnMut(B::Hash, &[u8]) -> bool + 'a> {
        let filter = self.gossip_filter.read();
        Box::new(
            move |_topic, mut data| match GossipMessage::<B>::decode_all(&mut data) {
...
pub(crate) mod tests {
    ...
    #[cfg(feature = "bls-experimental")]
    use sp_consensus_beefy_etf::bls_crypto::Signature;
    #[cfg(not(feature = "bls-experimental"))]
    use sp_consensus_beefy_etf::ecdsa_crypto::Signature;
    use sp_consensus_beefy_etf::{
        known_payloads, test_utils::Keyring, Commitment, MmrRootHash, Payload, SignedCommitment,
        VoteMessage,
    };
    ...
    pub fn sign_commitment<BN: Encode>(
        ...
        store
            .bls377_generate_new(BEEFY_KEY_TYPE, Some(&who.to_seed()))
            .unwrap();
        let beefy_keystore: BeefyKeystore<AuthorityId> = Some(store.into()).into();
        beefy_keystore
            .sign(&who.public(), &commitment.encode())
            .unwrap()
    }
    ...
    fn should_validate_messages() {
        ...
        let mut expected_report = PeerReport {
            who: sender,
            cost_benefit: expected_cost,
        };
        ...
        let set_id = vote.commitment.validator_set_id;
        assert_eq!(
            gv.gossip_filter.read().consider_vote(number, set_id),
            Consider::RejectFuture
        );
        let res = gv.validate(&mut context, &sender, &encoded);
        assert!(matches!(res, ValidationResult::Discard));
