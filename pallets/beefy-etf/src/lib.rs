...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config {
        ...
        /// Something that provides the round commitments for the current and next validator sets
        type RoundCommitmentProvider: RoundCommitmentProvider<Self::BeefyId, Self::MaxAuthorities>;
    }
    ...
    pub type ValidatorSetId<T: Config> =
        StorageValue<_, sp_consensus_beefy_etf::ValidatorSetId, ValueQuery>;
    ...
    pub type SetIdSession<T: Config> =
        StorageMap<_, Twox64Concat, sp_consensus_beefy_etf::ValidatorSetId, SessionIndex>;
...
impl<T: Config> Pallet<T> {
    ...
    fn change_authorities(
        ...
        if let Some(validator_set) =
            ValidatorSet::<T::BeefyId>::new(new, commitments.clone(), new_id)
        {
            ...
            if let Some(next_validator_set) =
                ValidatorSet::<T::BeefyId>::new(queued, commitments, next_id)
            {
