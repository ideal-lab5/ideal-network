...
{
    fn on_new_root(root: &sp_consensus_beefy_etf::MmrRootHash) {
        let digest = sp_runtime::generic::DigestItem::Consensus(
            sp_consensus_beefy_etf::BEEFY_ENGINE_ID,
            codec::Encode::encode(&sp_consensus_beefy_etf::ConsensusLog::<
...
impl Convert<sp_consensus_beefy_etf::ecdsa_crypto::AuthorityId, Vec<u8>> for BeefyEcdsaToEthereum {
    fn convert(beefy_id: sp_consensus_beefy_etf::ecdsa_crypto::AuthorityId) -> Vec<u8> {
...
impl Convert<sp_consensus_beefy_etf::bls_crypto::AuthorityId, Vec<u8>> for BeefyBlsToEthereum {
    fn convert(_beefy_id: sp_consensus_beefy_etf::bls_crypto::AuthorityId) -> Vec<u8> {
        Vec::new()
        // sp_core::ecdsa::Public::from(beefy_id)
        // 	.to_eth_address()
        // 	.map(|v| v.to_vec())
...
/// Type alias for the Merkle Root Hash.
